...
import {
  ...
  export default function NFT(props) {
    ...
    const { blockchain, setBlockchain, network, setNetwork, open_asset_page, setOpenAssetPage } = useContext(AccountContext);
          ...
          {winners && JSON.parse(winners).map((winner_group, index) => (
            ...
            >
              <Flex direction="column" mt="-15px">
                <Text color="gray.400" fontSize="18px" fontWeight="800" borderBottom="1px" borderColor="#11047A">
                  {`Epoch ${index + 1}`}
                </Text>
